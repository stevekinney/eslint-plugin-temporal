...
type MessageIds = 'datePayload';

type PayloadPosition = 'argument' | 'return';

function isDateTypeNode(node: TSESTree.Node): boolean {
...
export const noDateObjectInPayload = createWorkflowRule<Options, MessageIds>({
  name: 'workflow-no-date-object-in-payload',
  meta: {
    type: 'suggestion',
    docs: {
      description:
    ...
    messages: {
      datePayload:
        'Payload {{ position }} type "Date" is not JSON-serializable. Use ISO strings or epoch timestamps instead.',
    },
    schema: [
      {
        type: 'object',
        properties: {
          allowDate: {
            type: 'boolean',
            description: 'Allow Date types in payloads (for custom converters).',
          },
        },
  ...
  create(context, [options]) {
    ...
    function checkTypeNode(node: TSESTree.Node, position: PayloadPosition): void {
      walkTypeNodes(node, context.sourceCode, (entry) => {
        if (!isDateTypeNode(entry)) return;
